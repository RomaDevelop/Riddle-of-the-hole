...
void Value::PlusAssign(const IOperand *operand2)
{
	...
	else if(ValuesTypes::vectorsNumeric.contains(type) && ValuesTypes::simpleNumeric.contains(operand2->IOGetTypeValue()))
	{
		auto addingVal = operand2->IOGetValueAndType();
		auto valsList = Value::GetStringListFromValueVector(value);
		valsList += addingVal.first;
		value = valsList.join(ValuesTypes::vectorSplitter);
	}
	else Error("PlusAssign unrealesed PlusAssign. Tried to PlusAssign " + type + " to " + operand2->IOGetTypeValue());

	if(CheckCorrectionAndRange() != "")
	{
		Error("Value::PlusAssign error " + CheckCorrectionAndRange());
		value = "";
	}
}
...
int Value::GetSizeBitsOfType(const QString &type)
{
	...
	if(!sizeType) LogsSt::ErrorSt("GetSizeBitsOfType: can't get size of type ["+type+"]");
...
void Value::Set(QString newValue, QString type_)
{
	...
	if(cbValueChanged) cbValueChanged(this);
//	else if(type == type_) value = newValue;
//	else if(type == ValuesTypes::undefined) { value = newValue; type = type_; }
...
//			value = newValue;
//		}
//		else if(type_ == ValuesTypes::vectorUByte)
//		{
//			auto newValList {newValue.split(ValuesTypes::vectorSplitter, QString::SkipEmptyParts)};
//			int newValBitSize = newValList.size()*8;
//			if(type == ValuesTypes::bool_)
//			{
//				if(newValList.size() == 1 && newValList[0].toInt() <= 1)
//					value = QSn(newValList[0].toInt());
//				else Error("Set: error conversion. Tried to Set " + type_ + " in " + type + " new value = " + newValue);
//			}
//			else if(ValuesTypes::simpleNumeric.contains(type))
//			{
//				if(newValBitSize <= GetSizeBits())
//				{
...
//					}
//					value = QSn(newValList.join(ValuesTypes::vectorSplitter).remove(ValuesTypes::vectorSplitter).toUInt(nullptr,2));
//				}
//				else Error("Set: error conversion 1. Tried to Set " + type_ + " in " + type + " new value = " + newValue + " too big size");
//			}
//			else Error("Set: unrealesed conversion 2. Tried to Set " + type_ + " in " + type + " new value = " + newValue);
//		}
//		else Error("Set: unrealesed conversion 3. Tried to Set " + type_ + " in " + type + " new value = " + newValue);
//	}
//	else if(ValuesTypes::vectorsNumeric.contains(type) && ValuesTypes::vectorsNumeric.contains(type_)) value = newValue;
//	else Error("Set: unrealesed conversion 4. Tried to Set " + type_ + " in " + type + " new value = " + newValue);

//	if(!RangeCheck())
//	{
//		Error("Set: RangeCheck error. Tried to Set " + type_ + " in " + type + " new value = " + newValue);
//		value = "";
//	}
}
...
std::pair<Value,QString> Value::ConvertToType(QString newType) const
{
	...
	else if(type == ValuesTypes::text)
	{
		if(Compiler::IsInteger(value))
		{
			...
			else retErrorStr = "code 0.1 (tryed convert " + ToStrForLog() + " to " + newType + ")";
		}
		else if(Value::IsVectorNumeric(value) && ValuesTypes::vectorsNumeric.contains(newType))
		{
			retValue.value = value;
		}
...
QStringPair Value::BinCodeToVectorUByte(QString binCode)
{
	...
	for(int i=0; i<binCode.length(); i+=8)
	{
		...
		if(!ok) LogsSt::ErrorSt("Value::BinCodeToVectorUByte error. BinCode: " + binCode);
	}
...
QString Value::GetTypeByStrValue(const QString &value)
{
	...
	if(ret == ValuesTypes::unknown)
		LogsSt::ErrorSt("GetTypeByStrValue: can't get type of " +value);
	...
	if(Compiler::GetOperandName(dataOperand) == "Data")
	{
		...
		else if(indexesBytes.first.size() && indexesBytes.secnd.empty())
		{
			...
			else
			{
				...
				while(vectBytes.size() < indexesBytes.first.size())
					vectBytes.insert(vectBytes.begin(),Byte());

				if(vectBytes.size() == indexesBytes.first.size())
				{
					EasyData ed(msg);
		...
		// If seted 1 first and some secnd indexes
		else if(indexesBytes.first.size() == 1 && indexesBytes.secnd.size())
		{
			EasyData ed(msg);
			auto valueVectBits { ToVectBits() };
			while(valueVectBits.size() < indexesBytes.secnd.size()) valueVectBits.push_back(0);

			if(valueVectBits.size() == indexesBytes.secnd.size())
			{
		...
		else Error("ToDataUniversal error 8 dataOperand ["+dataOperand+"] can't be with 3 indexes");
	}
	else Error("ToDataUniversal error 9 wrong dataOperand " + dataOperand);
}
...
Value& Value::FromDataUniversal(const CANMsg_t &msg, QString dataOperand)
{
	...
	else if(indexes.first.size() == 1 && indexes.secnd.size())
	{
		if(ValuesTypes::simpleNumeric.contains(type))
		{
			...
			Byte byte;
			if(!byte.SetByte(msg.data[indexes.first[0]])) Error("FromDataUniversal: Error set Byte " + QSn(msg.data[indexes.first[0]]));
...
void EasyData::SetBitBi(int bitIndex, bool val)
{
	if(bitIndex > size)
	{
		LogsSt::ErrorSt("SetBit bitIndex ["+ QSn(size) +"] bigger size");
		return;
	}
...
bool EasyData::GetBit(int byteIndex, int bitInByteIndex)
{
	...
	{
		LogsSt::ErrorSt("EasyData::GetBit wrong index byteIndex = " + QSn(byteIndex) + " or bitInByteIndex = " + QSn(bitInByteIndex));
		return 0;
	}
}
...
{
	LogsSt::ErrorSt("FromBinCode unrealesed. Commented realisation contains errors." + strBinCode);
...
{
	len = msg.length;
	if(len > 8) { LogsSt::ErrorSt("FromMsg len > "+QSn(8)); return; }
	int eDataIndex = 0;
	for(int i=0; i<len; i++)
	{
		...
		{
			if(eDataIndex >= size) { LogsSt::ErrorSt("FromMsg eDataIndex >= " + QSn(size)); return; }
...
{
	msg.length = len;
	if(len > 8) { LogsSt::ErrorSt("ToMsg len > " + QSn(8)); return; }
	int eDataIndex = 0;
	for(int i=0; i<len; i++)
	{
		...
		for(int bit=0; bit<8; bit++)
		{
			if(eDataIndex >= size) { LogsSt::ErrorSt("ToMsg eDataIndex >= " + QSn(size)); return; }
...
void EasyData::FromVect(const std::vector<T> &vectVals)
{
	...
	if(len > 8 || vectVals.size() > size) LogsSt::ErrorSt("FromVect: to big size value" + QSn(vectVals.size()));
	for(uint i=0; i<vectVals.size(); i++)
	{
		if(vectVals[i] > 1 || vectVals[i] < 0) {
			data[i] = 0;
			LogsSt::ErrorSt("FromVect: to big value " + QSn(vectVals[i]));
		}
...
{
	auto indexes = Compiler::GetAllIndexes(strDataOperand);
	if(indexes.first.size() && indexes.secnd.empty())
	{
		for(uint i=0; i<indexes.first.size(); i++)
		{
			...
			else LogsSt::ErrorSt("SetDataFilterMaskFromOperand: err 1: to big index " + QSn(indexes.first[i]) + " in operand " + strDataOperand);
		}
	}
	else if(indexes.first.size() == 1 && indexes.secnd.size())
	{
		...
		for(uint i=0; i<indexes.secnd.size(); i++)
		{
			...
			else LogsSt::ErrorSt("SetDataFilterMaskFromOperand: err 2: to big index " + QSn(indexes.secnd[i]) + " in operand " + strDataOperand);
		}

		if(indexes.first[0] < 8) filter.mask[indexes.first[0]] = byte.to_ulong();
		else LogsSt::ErrorSt("SetDataFilterMaskFromOperand: err 3: to big index " + QSn(indexes.first[0]) + " in operand " + strDataOperand);
	}
	else LogsSt::ErrorSt("SetDataFilterMaskFromOperand: unrealesed indexes");
}
...
{
	Error("PlusAssign DataOperand " + operand2->IOGetTypeValue());
}

void DataOperand::Clear()
{
	Error("Clear: clearing DataOperand");
}
...
bool DataOperand::IsEqual(const IOperand *operand2) const
{
	bool retVal = false;
	QString type;
	if(operand2->IOGetClassName() == IOperand::classDataOperand()) // if compare DataOperand and DataOperand
		type = ValuesTypes::vectorBool;
	else type = operand2->IOGetTypeValue();
