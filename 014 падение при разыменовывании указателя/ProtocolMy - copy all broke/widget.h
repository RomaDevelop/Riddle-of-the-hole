...
class WidgetPair: public LogedClass
{
public:
	Widget view;
	Widget ctrl;
	...
	QString ToStr() const { return strIdParam+"|"+strParamDescribtion+"|"+strClassWindow+"|"+strCaption+"|"+strViewWidg+"|"
	...
	void SetAllToTextBrowser(QTextBrowser *tBrowser_, const QString & msgPrefix) override
	{
		...
		view.SetAllToTextBrowser(tBrowser_, msgPrefix);
		ctrl.SetAllToTextBrowser(tBrowser_, msgPrefix);
		for(auto &viewWidgetPart:view.parts)
			viewWidgetPart.SetAllToTextBrowser(tBrowser_, msgPrefix);
		for(auto &ctrlWidgetPart:ctrl.parts)
			ctrlWidgetPart.SetAllToTextBrowser(tBrowser_, msgPrefix);
	}
};
