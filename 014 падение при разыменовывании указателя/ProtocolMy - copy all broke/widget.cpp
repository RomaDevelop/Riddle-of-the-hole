...
//Value WidgetPart::GetWidgPartValue() const
//{
...
//	}
//	if(type == WidgetPartTypes::edit)
//	{
//		QString text = GetWidgPartCurrentText();
//		int errTr;
//		text = Transforms::Transform(name,transforms, text, Transforms::out, errTr);
...
//		bool ok;
//		text.toLongLong(&ok);
//		if(ok) return Value("",text,ValuesTypes::sLLong);
//		else { Error("GetWidgPartValue: wrong value: " + text); }
//	}
//	else Error("GetWidgPartValue: unrealesed type: " + type);
//	return Value();
//}
...
{
	QStringList commands { Compiler::TextToCommands(strWidgDescribtion) };
	for(auto cmnd:commands)
	{
		...
		else if(words[0] == CommandsKeyWords::transform)
		{
			...
			if(tr.name != "" && tr.firsts.size() && tr.secnds.size())
		...
		else Error("CreateWidget unnown command " + cmnd);
...
void Widget::SetWidgetValue(const Value &value)
{
	if(type == WidgetTypes::twoFieldBtn)
	{
		...
		auto vals = getValuePair.first.split(";");
		...
		else Error("SetWidgetValue for widget " + type + " wrong value " + IOperand::IOPairJoin(getValuePair));
	}
	else
	{
...
Value Widget::GetWidgetValue() const
{
	...
	if(type == WidgetTypes::chekBoxesNButton)
	{
		QString values;
		for(auto &p:parts)
			if(p.GetType() == WidgetPartTypes::checkBox) values += p.GetWidgPartCurrentText() + ";";
...
QColor WidgetColors::GetColor(QString colorStr)
{
	...
	if(all().contains(colorStr))
	{
		...
		else LogsSt::ErrorSt("GetColor wrong behavior");
	}
	else LogsSt::ErrorSt("GetColor unknown color "+colorStr);
	return ret;
}
...
int Transforms::FromQStringList(const QStringList &transformWords)
{
	...
	if(Check(transformWords))
	{
		...
		{
			firsts += TextConstant::GetTextConstVal(transformWords[i]);
			secnds += TextConstant::GetTextConstVal(transformWords[i+1]);
		}
	}
...
int Transforms::Add(Transforms transformsToAdd)
{
	...
	{
		firsts += transformsToAdd.firsts;
		secnds += transformsToAdd.secnds;
	}
...
QString Transforms::Transform(const QString &name, const Transforms &transforms, const QString &currText, int napravlenie, int &error)
{
	...
	if(transforms.firsts.size())
	{
		if(transforms.name == name)
		{
			for(int i=0; i<transforms.firsts.size(); i++)
			{
				if(napravlenie == in)
				{
					if(currText == transforms.firsts[i])
						ret = transforms.secnds[i];
				}
				else if(napravlenie == out)
				{
					if(currText == transforms.secnds[i])
						ret = transforms.firsts[i];
				}
			}
		}
...
void Window::CreateTabs(QLayout *loPlace)
{
	if(!this->tabWidget)
	{
		...
		for(uint widgI=0; widgI<this->widgets.size(); widgI++)
			if(!this->tabNames.contains(this->widgets[widgI].strTabName))
