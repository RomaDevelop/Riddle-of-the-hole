...
void ExcelWorkbook::Open(QString file)
{
...
{
	if(row > (int)rows.size() || col > rows[row-1].size()) qdbg<< "QString Cell(int col, int row)  out of boud";
...
{
	if(number > (int)sheetsPt.size()) qdbg<< "IExcelSheet* Sheet(int number) out of boud";
...
{
	qdbg << "Load from hashed file" << QFileInfo(fileName).fileName();
...
int MyQExcelHasher::InitFile1(QString directory, QString fileName)
{
	...
	else
	{
		...
		qdbg << "Hash not found, write new hash" << newFile;
