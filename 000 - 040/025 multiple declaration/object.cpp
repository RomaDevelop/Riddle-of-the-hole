...
Object::Object(const std::vector<QStringList> &paramsStrs, QString version, QString name_)
{
	if(version == "001")
	{
		...
		{
			params[i] = make_unique<Param>(paramsStrs[i], this);
		}
		...
		if(paramsCount)
		{
			...
			else Error("Object::Object error head category not inited");
		}
	}
...
void Object::AddParam(const QDomElement & paramElement)
{
	params.push_back(make_unique<Param>(paramElement,this));
	if(params.size() == 1)
	{
		auto headCat = CategoryPath::FirstCatFromPath(params[0]->CategoryCell());
		category.InitHead(headCat.name, headCat.number);
	...
	if(category.Inited())
	{
		auto catPath = CategoryPath::PathFromQString(params.back()->CategoryCell());
		category.InsertMember(*params.back().get(), catPath, true);
	}
	else Error("Object::AddParam error head category not inited");
}
