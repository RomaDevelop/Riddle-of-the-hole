...
IncludeParser::~IncludeParser()
{
	...
	for(unsigned int i=0; i<toSave.size(); i++)
		...
		else if (class_name=="QLineEdit") Settings += "toSave[s;]"+name+"[s;]"+class_name+"[s;]"+static_cast<QLineEdit*>(toSave[i])->text()+"[s;]";
		else if (class_name=="QTextEdit") Settings += "toSave[s;]"+name+"[s;]"+class_name+"[s;]"+static_cast<QTextEdit*>(toSave[i])->toPlainText()+"[s;]";
...
public:
	explicit CheckBoxDialog(QWidget *parent = nullptr) : QDialog(parent)
	{

	}

	bool Execute(const QStringList &strList)
	{
		//this->setLayout(new QVBoxLayout);
		//for(auto str:strList)
		//	this->layout()->addWidget(new QCheckBox(str));
...
void IncludeParser::on_pushButtonMassUpdate_clicked()
{
	...
	chDial->Execute(QStringList() << "1" << "2" << "3");
}
