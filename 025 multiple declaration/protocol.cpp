...
Protocol::Protocol(const std::vector<QStringList> &frameWorkersStrs, Object *objectToConnect_, QString version, QString name_, int type_, Constants *constants_,
				   int channel_, ICANChannel *chanPtr_):
	name {name_},
	...
	type {type_}
{
	if(version == "001")
	{
		...
		if(frWorkersCount)
		{
			...
			else Error("Protocol::Protocol error head category not inited");
		}
...
void Protocol::SendFrame(CANMsg_t &msg)
{
	if(chanPtr)
	{
		int res = chanPtr->WriteMsg(&msg);
	...
	if(emulatorCanNet)
	{
		...
		for(auto protInNet:network)
		{
			if(protInNet != this)
			{
				protInNet->msgs.push(msg);
				Log("send: " + msg.ToStrEx2(2));
			}
		}
	}
...
void Protocol::WorkerIncommingFrames()
{
	if(chanPtr)
	{
		while(chanPtr->GetRecCnt())
		{
			msgs.push(CANMsg_t());
			int res = chanPtr->ReadMsg(&msgs.back());
			if(res != 1) Error("Protocol::SendFrame ReadMsg error " + ICANChannel::ErrorCodeToStr(res));
		}
	}
	else Error("Protocol::SendFrame chanPtr is nullptr");
	...
	for(uint i=0; i<msgs.size(); i++)
	{
		...
		else
		{
			if(Settings::warningNotPassedFilterFrames)
				Warning("frame not passed filter: " + msgs.back().ToStrEx2(2));
		}
