...
Programm::Programm(IExcelWorkbook *wb, CANInterfaces *interfaces_)
{
	...
	if(version == "001")
	{
		...
		{
			if(FrameTypes::all.contains(paramsRows[i][Param_ns::Excel::type]))
			{
			...
			for(uint i=0; i<sizePSrc; i++)
			{
				...
				else
				{
					...
					else
					{
						...
						if(copyParamStr[cpci::category] != "") newParamStr[pci::Excel::category] = copyParamStr[cpci::category];
						if(copyParamStr[cpci::name] != "") newParamStr[pci::Excel::name] = copyParamStr[cpci::name];
						newParamStr[pci::Excel::id_] = QSn(idPDst[i]);
						if(copyParamStr[cpci::type] != "") newParamStr[pci::Excel::type] = copyParamStr[cpci::type];
		...
		else
		{
			...
			for(uint i=0; i<sizeId; i++)
			{
				...
				else
				{
					...
					if(copyWidgetStr[cwci::tabName] != "") newWidgetStr[wci::Excel::tabName] = copyWidgetStr[cwci::tabName];
					if(copyWidgetStr[cwci::tableName] != "") newWidgetStr[wci::Excel::tableName] = copyWidgetStr[cwci::tableName];
					newWidgetStr[wci::Excel::idWidget] = QSn(idWDst[i]);
					newWidgetStr[wci::Excel::idParam] = QSn(idPCon[i]);
...
void Programm::CreateControlPanel()
{
	...
	QCheckBox *chLogDoEvrythingCommands = new QCheckBox("Log DoEvrything Commands");
	...
	QCheckBox *chWarningNotPassedFilterFrames = new QCheckBox("Warning not passed filter frames");
	glo->addWidget(chWarningNotPassedFilterFrames,glo->rowCount(),1,1,-1);
	QObject::connect(chWarningNotPassedFilterFrames,&QCheckBox::clicked,[chWarningNotPassedFilterFrames](){
		if(chWarningNotPassedFilterFrames->isChecked()) Settings::warningNotPassedFilterFrames = true;
...
int Programm::FindIndexParamRow(QString idSrcParam, int startIndex)
{
	...
	{
		if(paramsRows[i][Param_ns::Excel::id_] == idSrcParam)
			return i;
	}
...
int Programm::FindIndexWidgetRow(QString windowClass, QString idSrcWidget, int startIndex)
{
	...
	{
		if(widgetsRows[i][Widget_ns::Excel::classWindow] == windowClass && widgetsRows[i][Widget_ns::Excel::idWidget] == idSrcWidget)
			return i;
	}
...
namespace  Programm_CreateConstsObjectsProtocols{
	int chNumber = -1;
}
void Programm::CreateConstsObjectsProtocols()
{
	for(auto &objRow:objectsRows)
	{
		...
		if(objRow[Object_ns::category] == Object_ns::emulator) tip = Protocol::emulator;
		else if(objRow[Object_ns::category] == Object_ns::servis) tip = Protocol::servis;
		else Error("Programm::Programm unknown genStr" + objRow.join(' '));

		QString name = objRow[Object_ns::name];
		int chanNumber = objRow[Object_ns::canChannel].toInt();
		namespace this_ns = Programm_CreateConstsObjectsProtocols;
		this_ns::chNumber = chanNumber;
		ICANChannel *chanPtr = nullptr;
		QString regimeStr = objRow[Object_ns::regime];
		QString baudStr = objRow[Object_ns::baud];
		int regime = Object_ns::Convert::GetICANRegime(regimeStr);
		int baud = Object_ns::Convert::GetICANBaud(baudStr);
		QString startSettings = objRow[Object_ns::startSettings];

		if(interfaces)
		{
			std::vector<ICANChannel*> channels;
			interfaces->GetChannels_if(channels, [](ICANChannel *ch){ return (ch->GetNumberAbsolut() == this_ns::chNumber); });
			if(channels.size() == 1)
			{
				chanPtr = channels[0];
				int res = chanPtr->SetRegime(regime);
				if(res == ICANChannel::NO_ERRORS)
				{
					res = chanPtr->SetBaud(baud);
					if(res == ICANChannel::NO_ERRORS)
					{
						res = chanPtr->Start();
						if(res == ICANChannel::NO_ERRORS)
						{
							Log("Channel "+QSn(chanNumber)+" for protocol " + name
								+ " started whith regime " + ICANChannel::RegimeToStr(chanPtr->GetRegime())
								+ " and baud " + ICANChannel::BaudToStr(chanPtr->GetBaud()));
						}
						else Error("Programm::CreateConstsObjectsProtocols Start error ("+ICANChannel::ErrorCodeToStr(res)+")");

					}
					else Error("Programm::CreateConstsObjectsProtocols SetBaud error ("+ICANChannel::ErrorCodeToStr(res)+")");
				}
				else Error("Programm::CreateConstsObjectsProtocols SetRegime error ("+ICANChannel::ErrorCodeToStr(res)+")");
			}
		}
		else Error("Programm::CreateConstsObjectsProtocols interfaces nullptr");

		Constants *consts = new Constants(contantsRows,version,startSettings);
		Object *obj = new Object(paramsRows, version, name);
		Protocol *prot = new Protocol(frameWorkersRows, obj, version,name,tip,consts, chanNumber, chanPtr);
...
void Programm::CreateWidgets()
{
	for(uint windowi=0; windowi<windows.size(); windowi++)
	{
		...
		{
			if(windows[windowi].classWindow == widgetsRows[i][Widget_ns::Excel::classWindow])
			{
				WidgetPair widget;

				widget.strIdToConnect = widgetsRows[i][Widget_ns::Excel::idParam];
				widget.strId = widgetsRows[i][Widget_ns::Excel::idWidget];
				widget.strCaption = widgetsRows[i][Widget_ns::Excel::caption];
				widget.strTabName = widgetsRows[i][Widget_ns::Excel::tabName];
				widget.strTableName = widgetsRows[i][Widget_ns::Excel::tableName];
				widget.strViewWidg = widgetsRows[i][Widget_ns::Excel::viewWidg];
				widget.strCtrlWidg = widgetsRows[i][Widget_ns::Excel::ctrlWidg];
...
void Programm::ConnectMake()
{
	for(auto &window:windows)
	{
		...
		{
			if(!window.parentProtocol)
			{
				Error("Programm::ConnectMake() window.parentProtocol is nullptr");
				continue;
			}
			...
			for(auto &wpt:widget.ctrl.parts)
			{
				if(wpt.GetType() == WidgetPartTypes::button)
				{
					...
					{
						if((protType == Protocol::servis && frWorker->TypeCell() == FrameTypes::inFrame)
								|| (protType == Protocol::emulator && frWorker->TypeCell() == FrameTypes::outFrame))
						{
